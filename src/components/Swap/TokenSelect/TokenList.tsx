...
import tokens, { defaultCustomTokens, sellLessTokens } from 'config/constants/tokens';
...
const TokenListItem = ({
    ...
    const { customTokens, tokenLogo } = useConfig();
    const isCustomToken = customTokens[token];
    const isDefaultToken = defaultCustomTokens[token];
        ...
        <ListItem disablePadding>
            <ListItemButton
                onClick={() => {
                    ...
                    switch (target) {
                        case 'base': {
                            if (detail.address !== tokens.bnb.address) {
                                onTokenSelect(tokens.bnb, 'quote');
                                onTokenSelect(detail, 'base');
                            } else if (detail.address === tokens.bnb.address) {
                                onTokenSelect(detail, 'base');
                                onTokenSelect(base, 'quote');
                            }
                            break;
                        }
                        case 'quote': {
                            if (detail.address !== tokens.bnb.address) {
                                onTokenSelect(tokens.bnb, 'base');
                                onTokenSelect(detail, 'quote');
                            } else if (detail.address === tokens.bnb.address) {
                                onTokenSelect(detail, 'quote');
                                onTokenSelect(quote, 'base');
                            }
                        }
                    }
                    onClose();
...
const TokenList = ({
    ...
    const { onImportNewToken, onRemoveCustomToken } = useConfig();
            ...
            <List sx={{ pt: 0 }}>
                {newToken.symbol && newToken.address !== sellLessTokens?.vfx?.address && (
                    <TokenListItem
                        ...
                        target={target}
                        isNewToken={true}
                        onImportNewToken={onImportNewToken}
                        setSearch={setSearch}
                    />
                )}
                {Object.entries(tokenList).map(([token, detail]: [string, any]) => {
                    const searchKey = search?.toLowerCase();
                    if (isFitBySearch(detail, searchKey))
                        return (
                            <TokenListItem
                                key={token}
                                tokenList={tokenList}
