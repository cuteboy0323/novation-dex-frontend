...
const SellLess = () => {
    ...
    const { SLSlippage: slippage, onChangeSLSlippage: onChangeSlippage, gasFee } = useConfig();
    ...
    const getCustomGasFee = useCallback(
        ...
        [getGasPrice, swapContract]
    );
    const getSlippage = useCallback(async () => {
        let slip = 0;
        if (slippage === 'auto') {
            const { buyFee, sellFee } = await swapContract.getTotalFee(
                isBuy() ? activeToken.quote.address : activeToken.base.address
            );
            slip = (isBuy() ? buyFee : sellFee) / 100 + 4;
        } else {
        ...
        slip = 100 - slip;
        return slip;
    }, [activeToken, slippage, swapContract]);
    ...
    const swap = useCallback(async () => {
        ...
        if (isBuy()) {
            ...
            const _token = activeToken.base.address;
            const _payable = toBigNumber(amount?.base?.wei).toFixed(0);
            const _slippage = slipAmount.toFixed(0);
            const method = 'sell';
            const args = [_token, _payable, _slippage];
            const gasOverrides = await getCustomGasFee(method, args);
            const receipt = await fetchWithCatchTxError(() => {
                return swapContract.sell(_token, _payable, _slippage, { ...gasOverrides });
            });
            if (receipt?.status) {
                checkAllowance();
                resetAmount();
    ...
    return (
        <Stack spacing={2} pt={1}>
            ...
            <TokenSelect
                ...
                tokenList={sellLessTokens}
            />
        </Stack>
    );
};

export default SellLess;
