...
const chainIds = NETWORK_LIST.map((item) => item.chainId);
...
export const walletconnect = new WalletConnectConnector({
    rpc: (() => {
        ...
        for (let i = 0; i < NETWORK_LIST.length; i++) {
            obj[NETWORK_LIST[i].chainId] = NETWORK_LIST[i].rpcUrl;
        }
        return obj;
    })(),
    qrcode: true,
...
export const bscConnector = new BscConnector({ supportedChainIds: chainIds });
...
export const connectorsByName = {
    ...
    [ConnectorNames.WalletLink]: async () => {
        ...
        return new WalletLinkConnector({
            url: rpcUrl,
            appName: 'TrueDefi',
            appLogoUrl: require('assets/img/logo.png'),
